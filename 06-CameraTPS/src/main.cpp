...
//std::shared_ptr<FirstPersonCamera> camera(new FirstPersonCamera());//-------------------->Creando apuntador para camara en primera persona
std::shared_ptr<Camera> camera(new ThirdPersonCamera());//--------------------------------->Declarando la camara en tercer persona
float distanceFromTarget = 5.0;//---------------------------------------------------------->Distancia de la camara al modelo
...
void scrollCallback(GLFWwindow* window, double xoffset, double yoffset);//--------------------->Para acercar o alejar la camara del pivote
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	glfwSetScrollCallback(window, scrollCallback);//----------------------->Declarando los eventos del scroll
	...
	camera->setPosition(glm::vec3(0.0, 0.0, 10.0));//---------------------------->Posición de la camara en primera persona
	camera->setDistanceFromTarget(distanceFromTarget);//------------------------->Para que calcule automaticamente la distancia de la camara
...
void scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {//------------------------------------->creando la función de la camara en tercera persona
...
return false;
	...
	//------------------------------------->Cambiando los métodos de los movimientos de la camara
...
void applicationLoop() {
	...
	while (psi) {
		...
		if (std::isnan(angleTarget))//------------------------->verificando que no este indeterminado
			angleTarget = 0;
		if (axis.y < 0)//------------------------->verificando si el angulo es negativo
			angleTarget = -angleTarget;
		if (modelSelected == 1)//------------------------->verificando que en el DartVader este detrás la camara
