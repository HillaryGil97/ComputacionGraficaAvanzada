...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Definiendo la textura a utilizar
	Texture textureTerrainBackground("../Textures/Texturas de Terreno/terreno.jpg");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
	...
	Texture textureTerrainBlendMap("../Textures/Texturas de Terreno/MiBlendMap.png");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
	bitmap = textureTerrainBlendMap.loadImage(true);//-----------------------------------> se agrego el true para blipear la textura
	// Convertimos el mapa de bits en un arreglo unidimensional de tipo unsigned char
...
bool processInput(bool continueApplication) {
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_RELEASE && glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_RELEASE
		...
		modelMatrixPacmanCorriendo = glm::translate(modelMatrixPacmanCorriendo, glm::vec3(0.0, 0.0, -0.08));
		modelMatrixPacmanDescanso = glm::translate(modelMatrixPacmanDescanso, glm::vec3(0.0, 0.0, -0.08));
		banderaPacman = 1;
	}
